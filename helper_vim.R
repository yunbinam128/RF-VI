...
library(FSelectorRcpp); #library(Rdimtools)
#library(smotefamily); #library(BayesFactor)

#source("../../aucm/R/auc.R") # load fast.auc function to calculate AUC
code.path <- '/Users/sxh3040/Desktop/Paper/SHan/1.VIM/Code/' # change to your path where codes are saved
source(paste(code.path, "auc.R", sep=''))
...
##### Appendix #####
# Proposed method
proposed.method2 <- function(res, data, Y, u=2, ntree, method=c('pAC-OOBER','pAUC-OOBER','pAC-CVER','pAUC-CVER','pAUC','pAUC+US','pAUC+OS','pAUC+ST')){
  
  ## Search stage
  vim.sorted <- res$vim.sorted
  vim.se.sorted <- res$vim.se.sorted
  ...
  if(method=='pAC-OOBER' | method=='pAUC-OOBER'){ # OOB-based
    for(i in 1:iter){
      #print(i)
      seleted.vars <- vim.sorted[1:candidate.vars[i]]
      reduced_data <- data[,c(Y, names(seleted.vars))]
      ...
      set.seed(123)
      fit.rf <- randomForest(factor(Y) ~ ., data=reduced_data, ntree=ntree, keep.inbag=TRUE, keep.forest=TRUE)
      oob.pred <- get.oob.pred(fit=fit.rf, data=reduced_data, Y=Y, perm=FALSE, metric='OOB-ER')
      pred.vec[i] <- mean(oob.pred[!is.infinite(oob.pred)])
      pred.se.vec[i] <- sd(oob.pred[!is.infinite(oob.pred)])/sqrt(length(oob.pred[!is.infinite(oob.pred)]))
    }
    
    # Select an optimal set of features based on OOB-ER
    best.idx <- which.min(pred.vec)
    best.pred <- pred.vec[best.idx]
    final.num.vars <- candidate.vars[best.idx]
    selected.vars <- names(vim.sorted[1:final.num.vars])
    selected.vars <- sort(selected.vars) # new
    
  } else if(method=='pAC-CVER' | method=='pAUC-CVER' | method=='pAUC' | method=='pAUC+US' | method=='pAUC+OS' | method=='pAUC+ST'){ # CV-based
    for(i in 1:iter){
      #print(i)
      seleted.vars <- vim.sorted[1:candidate.vars[i]]
      reduced_data <- data[,c(Y, names(seleted.vars))]
      ...
      kk <- 5
      cv.iters <- 1
      auc.vec <- rep(NA, cv.iters)
      for(cv.iter in 1:cv.iters){ # 1:100
        cv.idx <- get.kfold.splits(Y=reduced_data$Y, k=kk, seed=cv.iter*123)
        cv.pred.vec.temp <- rep(NA, kk)
        ...
        for(j in 1:kk){ # 5-fold CV
          ...
          if(method=='pAC-CVER' | method=='pAUC-CVER'){
            pred.Y <- predict(fit.rf, newdata=dat.test, type="response")
            conf.mat <- table(pred.Y, factor(dat.test$Y, levels=c(0,1)))
            cv.pred.vec.temp[j] <- (conf.mat[1,1]+conf.mat[2,2])/sum(conf.mat) # cv-ac
        ...
        auc.vec[cv.iter] <- mean(cv.pred.vec.temp)
      }
      
      pred.vec[i] <- mean(auc.vec[!is.infinite(auc.vec)])
    }
